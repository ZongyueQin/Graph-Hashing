...
flags.DEFINE_string('ground_truth_file', 'GT11.txt', '')
flags.DEFINE_string('laplacian', 'gcn', '')
flags.DEFINE_integer('GED_threshold', 11, 'upper bound for GED')
flags.DEFINE_string('special_note', 'scale exp', 'Special note.')
flags.DEFINE_string('node_label_name', 'label', 'Name of node label, none if it\'s idx')
flags.DEFINE_integer('real_pair_bs', 64, 'How many pairs in training is really sampled')
flags.DEFINE_integer('beam_width', 15, 'beam width for BSS-GED')
...
flags.DEFINE_string('dataset', 'AIDS', 'dataset')
...
if model in ['siamese_regression', 'siamese_ranking', 'siamese_matching']:
    ...
    flags.DEFINE_string(
        'layer_{}'.format(layer),
        'CNN:start_cnn=True,end_cnn=False,window_size=100,kernel_stride=1,'
    ...
    flags.DEFINE_string(
        'layer_{}'.format(layer),
        'CNN:start_cnn=False,end_cnn=False,window_size=25,kernel_stride=1,'
    ...
    flags.DEFINE_string(
        'layer_{}'.format(layer),
        'DeepSets:style={},gcn_num={}'.format(style, gcn_num), '')
    '''
...
flags.DEFINE_integer('gpu', 3, 'Which gpu to use.')  # -1: cpu
flags.DEFINE_integer('iters', 15, 'Number of iterations to train.')
flags.DEFINE_integer('iters_val_start', 1400,
                     'Number of iterations to start validation.')
flags.DEFINE_integer('iters_val_every', 200, 'Frequency of validation.')
if need_gc:
    flags.DEFINE_integer('iters_gc_start', 10000,
