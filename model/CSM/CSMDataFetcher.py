...
from CSM_utils import sorted_nicely

class CSMDataFetcher:
    ...
    def __init__(self, dataset, exact_ged = False):
        ...
        self.valid_percentage = CSM_FLAGS.csm_valid_percentage
        self.node_feat_type = CSM_FLAGS.csm_node_feat_encoder
        self.node_feat_name = CSM_FLAGS.csm_node_feat_name
        self.node_label_name = CSM_FLAGS.csm_node_label_name
    ...
    def samplePairListWithLabel(self, real_pair_batchsize, gen_pair_batchsize, 
                                transform=True):
        assert(real_pair_batchsize+gen_pair_batchsize==CSM_FLAGS.csm_batch_size)
        batchsize = int(np.ceil(real_pair_batchsize**0.5))
...
#        print(true_dist_sims) 
        ...
        self.k = int(np.ceil(gen_pair_batchsize/batchsize))
        if self.k > 0:
            pool = ThreadPool()
            map_pairs = [(i, g) for i,g in enumerate(self.sample_graphs)]        
            self.max_node = None
            pool.map(self.gen_sim_graph_wrp, map_pairs)
            pool.close()
            pool.join()
        
            p = 0
            for graphs, labs in zip(self.gen_graphs, self.gen_labels):
                for graph, lab in zip(graphs, labs):
                    if len(pairs) == CSM_FLAGS.csm_batch_size:
                        break
                    g1 = self.sample_graphs[p]
                    g2 = graph
                    pairs.append((g1, g2))
    ...
    def getNextTrainPair(self, transform=True):
        ...
        if self.cur_train_sample_ptr == len(self.train_graphs) - 1:
            self.cur_train_sample_ptr = 0
            shuffle(self.train_graphs)
        else:
            self.cur_train_sample_ptr = self.cur_train_sample_ptr + 1
            
        g2 = self.train_graphs[self.cur_train_sample_ptr]
        
        ged = self.getLabelForPair(g1, g2)
...
#        self._check_graphs_num(valid_graphs, 'validation')
    ...
    """ create a wrapper object (ModelGraph class) for each nxgraph """
    def create_wrapper_graph(self, graphs, tvt):
        ...
        for i, g in enumerate(graphs):
            mg = ModelGraph(g, self.node_feat_encoder)
            perc = i / len(graphs)
    ...
    def getLabelsForSampledGraphs(self, pair):
        ...
        if ged > -1:
            ...
            self.labels[id1, id2] = CSN_FLAGS.csm_GED_threshold
            self.labels[id2, id1] = CSM_FLAGS.csm_GED_threshold
        return
    ...
    def gen_sim_graph_wrp(self, pair):
        idx = pair[0]
        g = pair[1]
        gs, ls = self.generate_similar_graphs(g, self.k)
    ...
    def generate_similar_graphs(self, g, k):
        ...
        for i in range(k):
            ...
            generated_graphs.append(ModelGraph(tmp_g, self.node_feat_encoder))   
...
#        return self._sparse_to_tuple(inputs)
    ...
    def _preprocess_adj(self, adj_mat):
        ...
        if CSM_FLAGS.csm_laplacian == 'gcn': # what's other options ?
