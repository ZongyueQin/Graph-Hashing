...
def node_ordering(graph, method, node_feat_name, last_order=[]):
    ...
    if method == 'bfs':
        seq = [node for node in _bfs_seq(graph, nodes[0], node_feat_name, nodes, sort_ignore_nodes)]
    elif method == 'degree':
...
def _bfs_seq(graph, start_id, node_feat_name, sorted_nodes, sort_ignore_nodes=[]):
    ...
    dictionary = dict(_stable_bfs_successors(graph, start_id, sorted_nodes, sort_ignore_nodes))
    added = {ID:0 for ID in sorted_nodes}
    for ID in sort_ignore_nodes:
        added[ID] = 1
    cur_pos = 0
    start = []
    output = []
    ...
    while True:
        while cur_pos < len(sorted_nodes):
            if added[sorted_nodes[cur_pos]] == 0:
                break
            cur_pos = cur_pos + 1
        if cur_pos == len(sorted_nodes):
            break
        start.append(sorted_nodes[cur_pos])
        output.append(sorted_nodes[cur_pos])
        while len(start) > 0:
            next = []
            while len(start) > 0:
                current = start.pop(0)
...
def _stable_bfs_successors(graph, start_id, sorted_nodes, sort_ignore_nodes=[]):
...
#    queue.append(start_id)
    ...
    while True:
        ...
        if cur_pos == len(sorted_nodes):
            break
        queue.append(sorted_nodes[cur_pos])
        seen_nodes.add(sorted_nodes[cur_pos])
        while queue:
