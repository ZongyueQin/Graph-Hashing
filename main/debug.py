...
if __name__ == '__main__':
    ...
    candidate_fname = str(sys.argv[3])
    GED_thres = int(sys.argv[4])
...
#        ground_truth[q].append((g,d))
    ...
    f = open(candidate_fname, 'r')
    cand = {}
    for line in f.readlines():
        ids = line.split()
        q = int(ids[0])
        if q not in cand.keys():
            cand[q] = set()
#        print('q = %d'%q)
        for gid in ids[1:]:
#            print(gid, end = ' ')
...
#            pos = pos + 1
        ...
        candidate_set = cand[q]
        tmp2 = candidate_set & real_sim_set
        if len(tmp) < len(tmp2):
            print(q)
            print(tmp2-tmp)
            break
